...
print(date_list)
# 美国
x_amrican = xlrd.open_workbook("SP.xlsx")
sheet1 = x_amrican.sheet_by_name("plan")
am_confirm = sheet1.col_values(1, 2, 87)  # 取第2列，第1~71行（不含第5行）,返回一个list
# am_add = sheet1.col_values(2, 1, 72)
# print(len(am_add))
date_list_am = date_list
print(date_list_am)

t = range(len(confirm_list)-21)
t = np.array(t)
P = confirm_list[21:]
Q = am_confirm
...
future = range(len(confirm_list)-22, len(confirm_list)+5)
...
plt.title('西班牙新冠病毒发展趋势及预测', fontsize=20)
# 图像绘制
#plot_CN = plt.plot(t, P, '.', label="CN_Reality")
plot_US = plt.plot(t, Q, '.', label="西班牙当前实际数据")
plot2 = plt.plot(future, future_predict, 'r', label='西班牙未来预测数据')
...
plt.savefig('2019-nCoV_predict_sp.png')  # 保存为文件
plt.show()
