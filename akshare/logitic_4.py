...
popt1, pocv1 = curve_fit(logistic_increase_function_1, t, P)
...
P_predict_1 = logistic_increase_function_1(t, popt1[0], popt1[1], popt1[2])
...
plt.title('2019-nCoV_China_Predict', fontsize=20)
# 图像绘制
plot1 = plt.plot(t, P, '.', label="Reality")
plot2 = plt.plot(t, P_predict, 'r', label='maximum capacity with environmental resistance 1')
plot3 = plt.plot(t, P_predict_1, 'g', label='maximum capacity with environmental resistance 2')
plot4 = plt.plot(t, P_predict_2, 'y', label='maximum capacity with environmental resistance 3')
# 加入一条对比线
X1 = np.linspace(0, 88, 88)  # 构造自变量组
Y1 = [41000 for x in X1]  # 求函数值
plot5 = plt.plot(t, Y1)
# plot5 = plt.plot(tomorrow, tomorrow_predict, '.', label='future')
...
plt.plot(X, Y, 'b', label='maximum capacity without environmental resistance')  # 绘制指数函数
...
plt.savefig('2019-nCoV_cn_predict_k.png')  # 保存为文件
plt.show()
