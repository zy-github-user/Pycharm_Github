...
# 美国
x_amrican = xlrd.open_workbook("iran.xlsx")
sheet1 = x_amrican.sheet_by_name("plan")
am_confirm = sheet1.col_values(1, 2, 68)  # 取第2列，第1~71行（不含第5行）,返回一个list
# am_add = sheet1.col_values(2, 1, 72)
# print(len(am_add))
date_list_am = date_list

t = range(len(confirm_list)-39)
t = np.array(t)
P = confirm_list[39:]
Q = am_confirm
...
plt.title('2019-nCoV_Iran_Predict', fontsize=20)
# 图像绘制
#plot_CN = plt.plot(t, P, '.', label="CN_Reality")
plot_US = plt.plot(t, Q, '.', label="Iran_Reality")
#plot2 = plt.plot(t, P_predict, 'r', label='r = 0.265, k = 81810')
plot3 = plt.plot(t, P_predict_1, 'g', label='r = 0.265, k = 77731')
...
plt.savefig('2019-nCoV_predict_iran.png')  # 保存为文件
plt.show()
